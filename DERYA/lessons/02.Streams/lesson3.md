...
async function benchmarkHighWaterMark(filename, highWaterMark) {
  return new Promise((resolve) => {
    ...
    stream.on('end', () => {
      ...
      const mbPerSecond = (bytesRead / 1024 / 1024 / (duration / 1000)).toFixed(
        2,
      );
...
async function testErrorHandling() {
  ...
  try {
    ...
    assert.strictEqual(
      output.length,
      2,
      'Only 2 chunks processed before error',
    );
...
class DebugTransform extends Transform {
  ...
  _transform(chunk, encoding, callback) {
    ...
    console.log(
      `[${this.label}] Chunk #${this.chunkCount}: ${chunk.length} bytes`,
    );
  ...
  _flush(callback) {
    console.log(
      `[${this.label}] Finished: ${this.chunkCount} chunks, ${this.byteCount} bytes total`,
    );
    callback();
  }
}
...
async function retryablePipeline(streams, maxRetries = 3) {
  ...
  throw new Error(
    `Pipeline failed after ${maxRetries} attempts: ${lastError.message}`,
  );
}
...
**Stoic Philosophy**: 4. "Meditations" by Marcus Aurelius - Book 8, On accepting what cannot be changed 5. "Letters from a Stoic" by Seneca - Letter 16, On philosophy as a craft 6. "Discourses" by Epictetus - Book 1, Chapter 6, On providence and natural order

**Additional Resources**: 7. Stream Handbook: https://github.com/substack/stream-handbook 8. Node.js Best Practices: https://github.com/goldbergyoni/nodebestpractices

---

**END OF LESSON 3**

_Remember: The river does not resent its banks. The banks do not limit the river—they guide it, give it power, create its beauty. Similarly, constraints do not limit excellence; they define the path to it. Work within your limits. Optimize with measurement. Test with discipline. Deploy with confidence. This is the way of the skilled stream programmer, and the way of the Stoic craftsperson._
