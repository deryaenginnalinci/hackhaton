...
> _"Cling to someone else's wisdom as your own. You can learn from the living and the dead, but you must make the knowledge yours through practice."_
...
cart.on('itemAdded', (data) => {
  console.log(
    `✓ Added: ${data.item.name} x${data.quantity} ($${data.cartTotal})`,
  );
});
...
cart.on('quantityChanged', (data) => {
  console.log(
    `✓ Updated: ${data.item.name} ${data.oldQuantity} → ${data.newQuantity} ($${data.cartTotal})`,
  );
});
...
class DocumentWorkflow extends EventEmitter {
  constructor(documentId) {
    ...
    this.history = [
      { state: 'draft', timestamp: new Date(), reason: 'Created' },
    ];
...
doc.on('transitionRejected', (data) => {
  console.error(
    `✗ Transition rejected: ${data.from} → ${data.to} (${data.reason})`,
  );
});
...
**Stoic Philosophy**: 4. "Discourses" by Epictetus - On teaching and learning 5. "Letters from a Stoic" by Seneca - Letter 6, On sharing knowledge 6. "Meditations" by Marcus Aurelius - Book 6, On proper relationships with others

---

**END OF LESSON 2**

_Remember: The best teachers provide structure and wisdom, then trust students to apply it in their own way. The best event emitters announce what happens with clarity, then trust listeners to respond appropriately. Structure without domination, guidance without control._
